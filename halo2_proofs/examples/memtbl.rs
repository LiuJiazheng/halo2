...
impl MemTableConfig {
    ...
    fn configure(meta: &mut ConstraintSystem<Fr>) -> Self {
        ...
        meta.lookup("memory table range check", |meta| {
            ...
            vec![
                (addr_bit.clone(), binary_range),
                (acc_init_bit, binary_range),
                (acc_last_write_bit, binary_range),
                (cur_id, memory_range),
                (value, value_range),
                (cur_addr, memory_range),
                (addr_bit.clone() * delta_id, memory_range),
                ((one.clone() - addr_bit) * (delta_addr - one), memory_range),
            ]
        });
